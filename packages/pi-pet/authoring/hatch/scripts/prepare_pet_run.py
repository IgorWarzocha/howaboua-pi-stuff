#!/usr/bin/env python3
"""Create a Pi Pet run folder, prompts, and visual-generation job manifest."""

# Modified from the Apache-2.0 Hatch Pet workflow for Pi Pet and capability-neutral generation.
...
STATE_REQUIREMENTS = {
    ...
    "waiting": [
        "Show that Pi needs approval, help, or user input through an expectant asking pose.",
...
MAX_BRAND_DISCOVERY_BYTES = 256 * 1024
...
def infer_description(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    if args.pet_notes.strip():
        return sentence(f"A compact Pi Pet: {args.pet_notes}")
    if args.brand_name.strip():
        return sentence(f"A compact Pi Pet inspired by {args.brand_name}")
    if reference_paths:
        return "A compact Pi Pet based on the provided reference image."
    return "A compact original Pi Pet ready for animation."
...
def infer_pet_notes(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    return "a compact original Pi Pet"
...
def validate_reference(path: Path) -> None:
    ...
    if size <= 0 or size > MAX_REFERENCE_BYTES:
        raise SystemExit(f"reference must be 1-{MAX_REFERENCE_BYTES} bytes: {path}")
    try:
        with Image.open(path) as image:
            if image.width <= 0 or image.height <= 0 or image.width * image.height > MAX_REFERENCE_PIXELS:
                raise SystemExit(f"reference exceeds {MAX_REFERENCE_PIXELS} decoded pixels: {path}")
            image.verify()
    except SystemExit:
        raise
    except Exception as error:
        raise SystemExit(f"reference is not a supported image: {path}: {error}") from error
...
def base_pet_prompt(args: argparse.Namespace) -> str:
    ...
    return f"""Create one clean full-body reference sprite for Pi Pet {args.display_name}.
...
def row_prompt(args: argparse.Namespace, state: str, row: int, frames: int, purpose: str) -> str:
    ...
    return f"""Create one horizontal animation strip for Pi Pet `{args.pet_id}`, state `{state}`.
...
def retry_row_prompt(
    ...
    state_prompt = STATE_PROMPTS[state]
    state_requirements = "\n".join(f"- {line}" for line in STATE_REQUIREMENTS[state])
    return f"""Create Pi Pet row `{state}` for `{args.pet_id}`: exactly {frames} full-body frames in one horizontal strip on flat pure {chroma_name} {chroma_key}.
...
def look_row_prompt(
    ...
    return f"""Create one horizontal look-direction strip for Pi Pet `{args.pet_id}`, atlas row {row}.
...
def retry_look_row_prompt(
    ...
    chroma_key = args.chroma_key["hex"]
    chroma_name = args.chroma_key["name"]
    return f"""Create Pi Pet v2 look row {row} for `{args.pet_id}` as exactly 8 full-body frames in this order: {direction_list}.
...
def look_cardinal_prompt(args: argparse.Namespace) -> str:
    chroma_key = args.chroma_key["hex"]
    chroma_name = args.chroma_key["name"]
    return f"""Create one horizontal four-cardinal anchor strip for Pi Pet `{args.pet_id}`.
...
def look_cardinal_repair_prompt(
    ...
    return f"""Repair one cardinal anchor for Pi Pet `{args.pet_id}`: `{label}` means looking {expected_direction}.
...
def make_jobs(
    ...
    jobs: list[dict[str, object]] = [
        {
            ...
            "depends_on": [],
            "generation_capability": "image-generation",
    ...
    for state, _row, frames, _purpose in ROWS:
        ...
        if state == "running-left":
            ...
            derivation_policy = {
                ...
                "fallback_generation_capability": "image-generation",
            }
    ...
    jobs.append(
        {
            "id": "look-cardinals-approved",
            "kind": "visual-approval-gate",
            "status": "pending",
    ...
    for state, row, directions, _purpose in LOOK_ROWS:
        depends_on = (
            ["look-cardinals-approved"]
            if row == 9
            else ["look-cardinals-approved", "look-row-9"]
        )
        continuity_inputs = (
            []
            if row == 9
...
def main() -> None:
    ...
    if len(raw_reference_paths) > MAX_REFERENCE_COUNT:
        raise SystemExit(f"at most {MAX_REFERENCE_COUNT} references are allowed")
    ...
    if raw_brand_discovery_path is not None:
        ...
        if raw_brand_discovery_path.stat().st_size > MAX_BRAND_DISCOVERY_BYTES:
            raise SystemExit(f"brand discovery exceeds {MAX_BRAND_DISCOVERY_BYTES} bytes")
    ...
    request = {
        ...
        "primary_generation_capability": "image-generation",
    }
    ...
    (run_dir / "visual-jobs.json").write_text(json.dumps(jobs, indent=2) + "\n", encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "jobs": str(run_dir / "visual-jobs.json"),
                "ready_jobs": ["base"],
            },
            indent=2,
        )
