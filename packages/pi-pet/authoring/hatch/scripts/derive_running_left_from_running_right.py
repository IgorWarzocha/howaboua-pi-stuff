...
# Modified from the Apache-2.0 Hatch Pet workflow for Pi Pet's visual-jobs manifest.
...
def job_list(manifest: dict[str, object]) -> list[dict[str, object]]:
    ...
    if not isinstance(jobs, list):
        raise SystemExit("invalid visual-jobs.json: jobs must be a list")
...
def main() -> None:
    ...
    left_job["status"] = "staged"
    left_job["source_path"] = manifest_relative(output, run_dir)
    left_job["derived_from"] = "running-right"
    left_job["derived_from_source"] = manifest_relative(source, run_dir)
    left_job["staged_at"] = derived_at
    left_job["metadata"] = image_metadata(output)
    left_job["mirror_decision"] = {
        "approved": True,
        "approved_at": derived_at,
        "note": args.decision_note.strip(),
        "transform": "framewise-horizontal-mirror-preserving-order",
    }
    left_job.pop("completed_at", None)
    for key in [
        "last_error",
        "repair_reason",
        "queued_at",
