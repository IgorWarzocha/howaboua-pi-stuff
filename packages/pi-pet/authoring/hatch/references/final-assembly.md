...
```bash
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/make_contact_sheet.py" \
  "$RUN_DIR/final/spritesheet-extended.webp" --output "$RUN_DIR/qa/contact-sheet-extended.png"
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/make_direction_qa_sheet.py" \
  "$RUN_DIR/final/spritesheet-extended.webp" --output "$RUN_DIR/qa/look-directions.png"
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/make_direction_blind_qa_sheet.py" \
...
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/measure_direction_continuity.py" \
...
```bash
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/combine_direction_blind_verdicts.py" \
...
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/validate_direction_blind_verdicts.py" \
  --answer-key "$RUN_DIR/qa/direction-blind-answer-key.json" \
