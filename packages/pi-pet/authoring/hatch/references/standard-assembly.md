...
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/compose_atlas.py" \
...
uv run --project "$HATCH_DIR" --locked python "$HATCH_DIR/scripts/render_animation_previews.py" \
