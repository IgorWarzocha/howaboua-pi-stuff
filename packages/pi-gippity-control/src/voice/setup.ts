...
export function missingVoiceAudioSettings(
	config: GippityControlConfig,
	mode: "realtime" | "dictation",
): VoiceAudioSetting[] {
	return [
		...(!config.voice.inputDevice ? ["voice.inputDevice" as const] : []),
		...(mode === "realtime" && !config.voice.outputDevice
			? ["voice.outputDevice" as const]
			: []),
	];
}

export function formatVoiceAudioError(
	error: Error,
	mode: CodexVoiceMode,
	config: GippityControlConfig,
): string {
	...
	return [
		`GipPity could not open ${direction} device \`${device ?? "default"}\`.`,
...
function audioErrorDirection(
	...
	if (
		mode === "dictation" &&
		/(?:requested )?device|audio (?:device|stream)|capture stream|sample format/.test(
			normalized,
		)
	)
		return "input";
...
export function buildVoiceSetupInstructions(options: {
	...
	return [
		...
		`Explain the default controls: hold ${formatVoiceShortcut(DEFAULT_GIPPITY_CONTROL_CONFIG.voice.dictationShortcut)} to dictate and release to transcribe into Pi; ${formatVoiceShortcut(DEFAULT_GIPPITY_CONTROL_CONFIG.voice.realtimeShortcut)} toggles realtime voice; ${formatVoiceShortcut(DEFAULT_GIPPITY_CONTROL_CONFIG.voice.serverShortcut)} toggles the GipPity control server. Push mode follows key releases when available and key-repeat continuity otherwise; toggle behavior is selectable in /gippity. Keybinds and behavior can also be changed in ${options.configPath}; keybind changes take effect after /reload.`,
