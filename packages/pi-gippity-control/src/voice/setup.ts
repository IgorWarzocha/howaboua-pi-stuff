...
export function formatVoiceAudioError(
	...
	return [
		...
		`Agent: inspect the available audio devices and routes. Remove \`${setting}\` in \`${getGippityControlConfigPath()}\` to use the system default, or set an exact device id. For shared or processed audio, prefer the final virtual/system source rather than opening physical hardware directly. Then ask the user to try using voice features again.`,
...
export function buildVoiceSetupInstructions(options: {
	...
	const lines = [
		"GipPity voice audio setup was requested.",
