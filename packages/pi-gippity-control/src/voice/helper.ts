...
export class VoiceHelperClient {
	...
	async start(): Promise<void> {
		...
		if (!binary)
			throw new Error(
				`Codex voice helper is not bundled for ${process.platform}-${process.arch}`,
			);
		...
		const lines = new BoundedJsonlReader(
				...
				try {
					...
					if (event.type === "ready") {
						if (
							...
							ready.reject(
								new Error(
									`Unsupported Codex voice helper protocol ${event.version}`,
								),
							);
					}
			...
			() => {
				const error = new Error(
					"Codex voice helper emitted an oversized event",
				);
				ready.reject(error);
				this.fail(error);
		...
		child.once("exit", (code, signal) => {
			...
			const error = new Error(
				`Codex voice helper exited (${signal ?? code ?? "unknown"})${detail ? `: ${detail}` : ""}`,
			);
			ready.reject(error);
			if (this.child === child) {
				...
				new Promise<void>((_resolve, reject) => {
					timeout = setTimeout(
						() =>
							reject(
								new Error(
									`Codex voice helper did not become ready within ${READY_TIMEOUT_MS}ms`,
								),
							),
						READY_TIMEOUT_MS,
					);
	...
	send(command: VoiceHelperCommand): void {
		...
		if (
			child.stdin.writableLength + Buffer.byteLength(line) >
			MAX_HELPER_STDIN_BYTES
		) {
	...
	async stop(): Promise<void> {
		...
		try {
			...
			await Promise.race([
				...
				new Promise<void>((_resolve, reject) => {
					timeout = setTimeout(
						() => reject(new Error("Codex voice helper did not stop")),
						STOP_TIMEOUT_MS,
					);
				}),
			]);
...
function parseVoiceHelperEvent(value: unknown): VoiceHelperEvent {
	if (
		!value ||
		typeof value !== "object" ||
		!("type" in value) ||
	...
	if (
		event["type"] === "devices" &&
		validDevices(event["inputs"]) &&
		validDevices(event["outputs"])
	)
		return event as VoiceHelperEvent;
	if (
		event["type"] === "offer" &&
		boundedString(event["sdp"], MAX_REALTIME_SDP_BYTES)
	)
		return event as VoiceHelperEvent;
	...
	if (
		event["type"] === "data" &&
		boundedJson(event["message"], MAX_DATA_MESSAGE_BYTES)
	)
		return event as VoiceHelperEvent;
	if (
		event["type"] === "pcm" &&
	...
	if (
		event["type"] === "error" &&
		boundedString(event["message"], MAX_TEXT_BYTES)
	)
		return event as VoiceHelperEvent;
