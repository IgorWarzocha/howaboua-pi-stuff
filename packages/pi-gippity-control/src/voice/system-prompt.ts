...
const CODEX_VOICE_PROMPT_VERSION_MARKER =
...
const REQUIRED_CODEX_VOICE_SECTIONS = [
	"Interface and role",
	"Delegation",
	"Session continuity",
	"Backend results",
...
export function getCodexVoiceSystemPromptPath(
...
export function ensureCodexVoiceSystemPrompt(
	promptPath: string = getCodexVoiceSystemPromptPath(),
): { created: boolean } {
	mkdirSync(dirname(promptPath), { recursive: true });
	try {
		writeFileSync(promptPath, DEFAULT_CODEX_VOICE_SYSTEM_PROMPT, {
			encoding: "utf8",
			flag: "wx",
			mode: 0o600,
		});
...
export function loadCodexVoiceSystemPrompt(
	...
	const missing = REQUIRED_CODEX_VOICE_SECTIONS.filter(
		(section) => !hasMarkdownSection(prompt, section),
	);
	if (missing.length > 0) {
		throw new Error(
			`GipPity prompt at ${promptPath} is missing required section${missing.length === 1 ? "" : "s"}: ${missing.join(", ")}`,
		);
	}
...
function migrateCodexVoiceSystemPrompt(promptPath: string): void {
	...
	const section = hasMarkdownSection(prompt, "Session continuity")
		? ""
		: appendSection(source, newline);
	writeFileSync(
		promptPath,
		`${CODEX_VOICE_PROMPT_VERSION_MARKER}${newline}${source}${section}`,
		"utf8",
	);
}
...
function readVoicePrompt(
	...
	try {
		...
		throw new Error(
			`Could not read Codex voice prompt at ${promptPath}: ${errorMessage(error)}`,
		);
	}
	let prompt: string;
	try {
		...
		throw new Error(
			`Invalid Codex voice prompt at ${promptPath}: ${errorMessage(error)}`,
		);
	}
	return prompt;
}
...
function stripMarkdownComments(source: string): string {
	...
	while (cursor < source.length) {
		...
		if (strayClosing !== -1 && (opening === -1 || strayClosing < opening)) {
			throw new Error(
				"Codex voice prompt contains an unmatched Markdown comment close",
			);
		}
		if (opening === -1) {
		...
		if (closing === -1)
			throw new Error(
				"Codex voice prompt contains an unclosed Markdown comment",
			);
		cursor = closing + 3;
	}
	return output
