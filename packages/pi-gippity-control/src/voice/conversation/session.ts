import { ProxyAgent, type Response, fetch as undiciFetch } from "undici";
	...
	private flushHandoff(): void {
		...
		try {
			for (const text of utf8Chunks(this.handoffBuffer, HANDOFF_CHUNK_BYTES)) {
				this.peer.sendData({
					type: "delegation.context.append",
					delegation_item_id: this.activeDelegationId,
					channel: this.handoffChannel,
					content: [{ type: "input_text", text }],
				});
			}
			this.handoffBuffer = "";
...
async function setupRealtimeCall(
	endpoint: string,
	headers: Headers,
	signal: AbortSignal,
	body: string,
	...
	try {
		...
		return {
			...
			answer: await readBoundedResponseText(response, MAX_REALTIME_SDP_BYTES),
		};
	} finally {
