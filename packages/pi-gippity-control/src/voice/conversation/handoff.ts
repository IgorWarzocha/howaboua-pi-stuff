...
export class RealtimeDelegationHandoff {
	...
	constructor(
		...
		callbacks: RealtimeDelegationHandoffCallbacks,
	) {
		this.peer = peer;
	...
	mirrorPiSteer(input: unknown): boolean {
		...
		this.flush();
		if (!this.callbacks.isActive() || this.activeDelegationId !== delegationId)
			return false;
		try {
	...
	stream(type: string, delta: string): void {
		...
		this.buffer += delta;
		if (!this.timer)
			this.timer = setTimeout(() => this.flush(), HANDOFF_FLUSH_MS);
	}
	...
	flush(): void {
		...
		if (!this.callbacks.isActive() || !this.activeDelegationId || !this.buffer)
			return;
		try {
