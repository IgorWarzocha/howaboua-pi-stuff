...
The LAN server includes a microphone mute button. Realtime audio uses the browser's native WebRTC playback while the server retains authenticated setup and Pi delegation. The server is unauthenticated by design for trusted networks, uses a local HTTPS certificate, belongs only to the Pi session that started it, and stops when that session changes.
