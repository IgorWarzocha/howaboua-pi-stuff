...
- `functions` grouping belongs to Responses Lite. Context windows independently expose `history.*` and `notes.*` namespaces where plaintext or remote-encrypted arguments are viable; stream and replay adapters map them to Pi's flat internal routers. Codex-local fallback and unrelated standard tools stay flat.
