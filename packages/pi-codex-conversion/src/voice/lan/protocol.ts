...
export type LanVoiceAudioCommand =
	...
	| { type: "peer_state"; state: "connected" | "disconnected" | "failed" | "closed" | "ready" }
...
export function decodeLanVoiceAudioCommand(value: unknown): LanVoiceAudioCommand {
	...
	if (value.type === "start") {
		...
		if (!("sdp" in value) || !boundedString(value.sdp, MAX_REALTIME_SDP_BYTES)) throw invalidCommand();
	...
	if (value.type === "peer_data") {
		if (!("message" in value) || !boundedJson(value.message, MAX_PEER_DATA_BYTES)) throw invalidCommand();
		return { type: "peer_data", message: value.message };
	}
	if (value.type === "peer_error") {
		if (!("message" in value) || !boundedString(value.message, MAX_PEER_ERROR_BYTES)) throw invalidCommand();
...
function boundedJson(value: unknown, maxBytes: number): boolean {
	try { return Buffer.byteLength(JSON.stringify(value)) <= maxBytes; } catch { return false; }
}
