...
const LINUX_X64_ARCHIVE_BYTES = 41_638_854;
...
export function resolveDenoAsset(platform: string, arch: string): readonly [string, string, number, string, number] {
	if (platform !== "linux" || arch !== "x64") {
		throw new Error(`Notebook Code Mode prototype supports linux-x64 only; got ${platform}-${arch}`);
	}
	return [LINUX_X64_ASSET, LINUX_X64_SHA256, LINUX_X64_ARCHIVE_BYTES, LINUX_X64_BINARY_SHA256, LINUX_X64_BINARY_BYTES];
}
