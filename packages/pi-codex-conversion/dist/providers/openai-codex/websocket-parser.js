import { extractWebSocketCloseError, extractWebSocketError } from "./websocket-connection.js";
...
export async function* countWebSocketEvents(events, onEvent) {
    for await (const event of events) {
        onEvent();
        yield event;
    }
}
export async function* startWebSocketOutputOnFirstEvent(events, output, stream, onStart) {
    let started = false;
    for await (const event of events) {
        if (!started) {
            started = true;
...
export function isRetryableEarlyWebSocketError(error) {
    const message = error instanceof Error ? error.message : String(error);
    if (/message too big/i.test(message))
        return false;
