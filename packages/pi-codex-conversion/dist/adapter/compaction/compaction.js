...
import { extractCompactionSummaryText, hasCompactionOutputItem, sanitizeCompactedWindow, summarizeCompactionOutputForDiagnostics } from "./compaction-output.js";
import { findLatestNativeCompactionEntry, findLatestNativeCompactionEntryIndex, resolveLatestNativeCompactionEntry } from "./details-store.js";
import { shrinkNativeCompactionRequestForEndpoint } from "./request-shrink.js";
...
import { formatCodexUsageLimitError } from "../../providers/openai-codex/errors.js";
import { convertResponsesTools } from "../../providers/openai-responses/shared.js";
import { serializeCompactionPreparationToRequest, } from "./serializer.js";
import { createNativeCompactionDetails, createNativeCompactionShimResult, isNativeCompactionDetails, NATIVE_COMPACTION_SHIM_SUMMARY } from "../compaction/types.js";
import { applyResponsesLiteRequest, prepareResponsesLiteRequestImages, supportsResponsesLiteModel } from "../../providers/openai-codex/responses-lite.js";
import { isResponsesContext } from "../prompt/codex-model.js";
import { isEffectiveOpenAICodexContext, shouldUseNativeResponsesCompaction } from "../activation/activation.js";
...
function buildCompactionInstructions(systemPrompt, customInstructions) {
...
function buildCompactionTools(pi, ctx, state) {
    void ctx;
    void state;
...
function buildCompactionReasoning(pi, ctx, state, compactionModel) {
    ...
    if (!model?.reasoning || level === "off")
        return undefined;
    const clampedLevel = clampThinkingLevel(model, level);
...
function buildCompactionRequestOptions(pi, ctx, state, compactionModel) {
...
function getCompactionIdentity(entry) {
...
function formatCompactFailureMessage(compactResult) {
    if (compactResult.ok)
        return "OpenAI native compaction succeeded";
    ...
    if (friendly)
        return `OpenAI native compaction failed (${compactResult.reason}${status}): ${friendly}`;
...
function formatCompactRequestDiagnostics(request) {
...
async function handleCodexSessionBeforeCompactInner(event, ctx, state, pi) {
    ...
    if (responsesLite)
        request = await prepareResponsesLiteRequestImages(applyResponsesLiteRequest(request));
...
export async function rewriteCodexCompactedProviderRequest(payload, ctx, state) {
    if (!shouldUseNativeResponsesCompaction(ctx, state.config) || (!isEffectiveOpenAICodexContext(ctx, state.config) && !isResponsesContext(ctx)))
        return undefined;
    ...
    if (!runtime.payload)
        return undefined;
    const rewrite = rewriteResponsesPayloadWithNativeReplay({ model: runtime.currentModel, payload: runtime.payload, branchEntries, compactionEntry: branchEntries[latestNativeCompactionIndex] });
    if (rewrite.ok)
