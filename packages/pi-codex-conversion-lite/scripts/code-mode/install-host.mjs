...
if (!asset) {
	console.error(`[pi-codex-conversion-lite] Unsupported code-mode platform: ${platform}`);
	process.exit(1);
}
...
if (!destination || basename(destination) !== binaryName) {
	console.error(`[pi-codex-conversion-lite] Usage: install-host.mjs /absolute/path/${binaryName}`);
	process.exit(1);
}
...
try {
	...
	console.log(`[pi-codex-conversion-lite] Installed ${destination}`);
} finally {
