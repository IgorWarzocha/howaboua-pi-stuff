...
test("stopping dictation aborts only an in-flight WebSocket setup", async () => {
	...
	assert.ok(helperCloses >= 1);
	assert.equal((session as unknown as { state: string }).state, "failed");
});

test("dictation start rejects when the session fails during WebSocket setup", async () => {
	const helper = {
		onEvent: () => () => {},
		onExit: () => () => {},
		start: async () => {},
	...
	const internal = session as unknown as {
		...
		connector: () => Promise<{ close(): void }>;
	};
	internal.helper = helper;
	internal.connector = async () => {
		internal.fail(new Error("helper failed during setup"));
		return { close: () => { socketCloses += 1; } };
	};
