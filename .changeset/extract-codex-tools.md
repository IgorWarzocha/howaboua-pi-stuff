---
"@howaboua/pi-codex-conversion": patch
"@howaboua/pi-codex-imagegen": patch
"@howaboua/pi-codex-web-run": patch
---

Make Codex web search and image generation independently installable TypeScript extensions.

- Pi Codex keeps the minimum shell, patch, image-inspection, Code Mode, and Notebook Mode environment.
- Web search preserves native alpha/search navigation and reusable references without platform binaries.
- Image generation supports generation plus local or recent-image edits and workspace-local artifacts.
- Both extensions remain normal Pi tools and automatically compose into Code and Notebook Mode.
- A shared JSON route file supports renamed or proxied Codex providers and model aliases.
- Local edits validate complete image structure, and outdated Pi Codex installations receive an explicit update error.
- Nested custom renderer state stays within a session-wide byte budget.
