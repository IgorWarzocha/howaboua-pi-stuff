...
2. Fetch `origin/main` once and record its SHA. Create one worker branch/worktree per issue from that SHA, outside the canonical checkout under a sibling root such as `<repo-parent>/.worktrees/<repo>/issue-123`.
3. Require `HERDR_ENV=1` before controlling panels. Create one unfocused Herdr workspace per worktree, label it with the issue number and short title, and launch a named Pi session with `--model openai-codex/gpt-5.6-luna:high`. Parse workspace and pane IDs from Herdr's JSON; do not guess IDs.
4. Give each worker this ownership contract: read the issue and repository instructions; implement only that issue; run focused validation; cull weak tests; commit all intended work; do not push, open a PR, add a changeset, run the umbrella gate, or touch another worktree; return commit SHA, changed surface, checks, and risks.
5. Use `herdr_agent` for ordinary send, wait, and read operations. Let workers finish independently; steer scope or answer blockers without polling. Review only committed worker results.
6. In a dedicated coordinator worktree at the recorded base, inspect each worker commit and return incorrect or mixed work for repair. Create the bottom stack branch with `gh stack init`, cherry-pick its accepted commit, then use `gh stack add` and cherry-pick each remaining commit in dependency/conflict order. Resolve shared-file integration centrally.
7. Add a final release layer to the top. Run patch-autodetecting `bun changeset -- "<combined release summary>"` there so the stack carries one release artifact, then apply the verification cull across the complete stack and run the umbrella gate once from the top.
8. Run `gh stack submit --auto --open`, then verify and edit every PR's title, body, issue linkage, base, and order. Use `Closes` only where the layer fully resolves its issue. Request review per repository policy.
9. When every layer is approved and green, confirm the target does not require a merge queue, then directly merge the top with `gh stack merge <top-pr> --yes` and the repository's merge method. Verify the atomic `main` landing and release workflow; until one live batch confirms event behavior, do not claim the one-run release property as measured fact.
10. Keep worker branches, worktrees, and panels until the stack no longer needs follow-up. Never remove a worktree with uncommitted changes.
